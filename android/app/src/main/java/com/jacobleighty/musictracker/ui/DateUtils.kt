...
object DateUtils {
    ...
    fun parseDate(s: String): LocalDate {
        ...
        return try {
            when (parts.size) {
                3 -> LocalDate.of(parts[2].toInt(), parts[0].toInt(), parts[1].toInt())
                2 -> LocalDate.of(parts[1].toInt(), parts[0].toInt(), 1)
                else -> LocalDate.of(parts[0].toInt(), 1, 1)
            }
        } catch (_: Exception) { LocalDate.now() }
    }

    fun getYear(s: String): String = s.trim().split("/").last()
    ...
    fun parseParts(s: String): DateParts {
        ...
        return try {
            when (parts.size) {
                3 -> DateParts(MONTHS[parts[0].toInt() - 1], parts[1].toInt(), parts[2].toInt())
                2 -> DateParts(MONTHS[parts[0].toInt() - 1], null, parts[1].toInt())
                else -> DateParts(null, null, parts.firstOrNull()?.toIntOrNull())
            }

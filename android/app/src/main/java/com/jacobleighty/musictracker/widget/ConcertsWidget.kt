...
class ConcertsWidget : GlanceAppWidget() {
    ...
    override suspend fun provideGlance(context: Context, id: GlanceId) {
        ...
        provideContent {
            ...
            ConcertsWidgetContent(upcoming, twoColumn, singleColumn)
        }
    }
...
class ToggleConcertsColumnAction : ActionCallback {
    override suspend fun onAction(context: Context, glanceId: GlanceId, parameters: ActionParameters) {
        updateAppWidgetState(context, PreferencesGlanceStateDefinition, glanceId) { prefs ->
            prefs.toMutablePreferences().also {
                it[ConcertsWidget.SINGLE_COLUMN_KEY] = !(it[ConcertsWidget.SINGLE_COLUMN_KEY] ?: false)
            }
        }
        ConcertsWidget().update(context, glanceId)
    }
}

@Composable
private fun ConcertsWidgetContent(concerts: List<Concert>, twoColumn: Boolean, singleColumn: Boolean) {
    ...
    Column(
        ...
        Row(
            ...
            Text(
                ...
                modifier = GlanceModifier
                    .padding(end = 4.dp)
                    .clickable(actionRunCallback<ToggleConcertsColumnAction>()),
            )
        }
        if (concerts.isEmpty()) {
        ...
        if (twoColumn) {
            val chunks = concerts.chunked(2)
            LazyColumn(modifier = GlanceModifier.fillMaxSize()) {
                items(chunks, itemId = { row -> row.first().id.toLong() }) { rowConcerts ->
                    Row(
                        modifier = GlanceModifier.fillMaxWidth().padding(vertical = 3.dp),
                        horizontalAlignment = Alignment.Horizontal.Start,
                    ) {
                        rowConcerts.forEach { concert ->
                            ConcertCard(concert, accent, modifier = GlanceModifier.defaultWeight().padding(horizontal = 3.dp), large = false)
                        }
                        if (rowConcerts.size == 1) Spacer(GlanceModifier.defaultWeight())
                    }
                }
            }
        } else {
            LazyColumn(modifier = GlanceModifier.fillMaxSize()) {
                items(concerts, itemId = { it.id.toLong() }) { concert ->
                    ConcertCard(concert, accent, modifier = GlanceModifier.fillMaxWidth().padding(vertical = 4.dp), large = true)
                }
            }
        }
    }
...
@Composable
private fun ConcertCard(concert: Concert, accent: ColorProvider, modifier: GlanceModifier = GlanceModifier, large: Boolean = true) {
    ...
    Column(modifier = modifier) {
        Row(
            ...
            Column(modifier = GlanceModifier.defaultWeight()) {
                Text(concert.band, style = TextStyle(color = textPri, fontSize = nameFontSize, fontWeight = FontWeight.Bold), maxLines = 1)
                if (concert.venue.isNotEmpty()) {
                    Text(concert.venue, style = TextStyle(color = textSec, fontSize = subFontSize), maxLines = 1)
                }
            }
        }
    }
